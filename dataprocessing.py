...
def excel_to_txt_files(excel_file_path, output_file_dir):
    """
    This function will take in an input data spreadsheet and output a folder of .txt files with the data - one file per category. 
    This is to organize the data before tokenization.
    The output will be split by category: synonyms, pop_culture, thematic, word_based, fitb (fill in the blank)
    ...
    if not os.path.exists(output_file_dir):
        logging.debug(f"Output file dir: {output_file_dir} does not exist. Creating file in {os.getcwd()}/preprocessed_data")
        os.mkdir(f"{os.getcwd()}/preprocessed_data")
        output_file_dir = f"{os.getcwd()}/preprocessed_data"
    ...
    for _, row in df.iterrows():
        if row[0] in categories:        
            if category_dictionary != {}:
                output_file = os.path.join(output_file_dir, f"{heading}.txt")
                with open(output_file, 'a') as outfile:
                    outfile.write(json.dumps(category_dictionary, indent=2))
            category_dictionary = {}
            heading = row[0]
        else:
            string_split = row[0].split('"')
...
def excel_to_flan_format(excel_file_path, output_file_dir):
    """
    This function will take in an input data spreadsheet and output a folder of .txt files with the data - one file per category. 
    This is to organize the data before tokenization in the format for flan t5 hugging face notebook
    ...
    for _, row in df.iterrows():
        if row[0] in categories:        
            if category_dictionary != {}:
                output_file = os.path.join(output_file_dir, f"{heading}_FLAN.txt")
                with open(output_file, 'a') as outfile:
                    outfile.write("train=")
                    outfile.write(json.dumps(category_dictionary, indent=2))
            category_dictionary = {}
...
if __name__ == "__main__":
    #excel_to_txt_files("./original_dataset.xlsx", "./preprocessed_data")
    excel_to_flan_format("./original_dataset.xlsx", "./preprocessed_data")
